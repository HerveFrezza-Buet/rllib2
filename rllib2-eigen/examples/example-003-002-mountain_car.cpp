...
void print_context(const std::string& msg,
		   gdyn::problem::mountain_car::system::observation_type& obs,
		   double reward ) {
...
int main(int argc, char* argv[]) {
  ...
  auto simulator = gdyn::problem::mountain_car::make();
  // Let us check the type requirements for our simulator.
  static_assert(gdyn::concepts::system<decltype(simulator)>);
  
  simulator = gdyn::problem::mountain_car::random_state(gen, simulator.param); // We set the state.
  ...
  // one.
  auto reward = simulator(gdyn::problem::mountain_car::random_command(gen));
  ...
  for(auto command
	: gdyn::views::pulse([&gen](){return gdyn::problem::mountain_car::random_command(gen);})
	| std::views::take(20))
  ...
  auto policy = [](const gdyn::problem::mountain_car::system::observation_type obs) {
    // Left if positive theta, Right otherwise
    if (obs.velocity > 0) return gdyn::problem::mountain_car::system::command_type::Right; // We can use system::command_type...
    else               return gdyn::problem::mountain_car::acceleration::Left;           // ... or directly gdyn::problem::mountain_car::acceleration
    
  };

  // change Simulator parameters
  simulator.param.gravity = 0.0;
  ...
  for(auto command
    ...
    obs = *simulator;
    print_context(gdyn::problem::mountain_car::to_string(command)+" => ", obs, reward );
