...
namespace rl2 {
  namespace eigen {
    ...
    auto to_eigen(const N& ns, const A& a) {
      // TODO some have:: dim, other ::size())
      Eigen::Vector<double, N::dim*A::size()> vec;
      vec.setZero(); // TODO needed ?

      std::cout << "__to_eigen of a size " << N::dim << " vector" << std::endl;
  ...
  std::string shape (const M& mat)
  {
    ...
    for (auto it=begin; it!=end; ++it) {
      ...
      //      ou *(it).s
          auto phi_s_eigen = to_eigen((*q.s_feature)(it->s), it->a);
          std::cout << "**** Transition " << count_trans << std::endl;
          std::cout << " phi_s = " << phi_s_eigen << std::endl;
          auto phi_ss_eigen = to_eigen((*q.s_feature)(it->ss), pi(it->ss));
